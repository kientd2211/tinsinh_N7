...
def main():
    ...
    for samp in samples:
        # Sửa ở đây
        clean_path = samp.path.replace('\\', '/').split('/')[-1]
        
        samp.pdb.write(os.path.join(inf_args.model_dir, inf_name, clean_path + f".{samp.copy}.anim.pdb"), reverse=True)
        samp.pdb.clear().add(samp.Y).write(
            os.path.join(inf_args.model_dir, inf_name, clean_path + f".{samp.copy}.pdb"))
