...
def reverse_sample(args, score_func, sde, sched, pdb=None, Y=None, device='cpu', tqdm_=True, ode=False, logF=None):
    ...
    for i in steps:
        ...
        # 1. Trả score về lại định dạng NumPy
        ...
        if logF is not None:
            pass # Bỏ qua tính toán logF phức tạp khi chạy demo bằng CPU
        
        # 2. Dùng numpy để sinh nhiễu thay vì torch.randn
        noise = np.random.randn(*Y.shape)
        dY = dY + np.sqrt(dt*(1+sched.alpha)) * noise
        
        Y = Y + sde.project(dY, k, center=False)
        
        if pdb: pdb.add(Y)
    
    # 3. Trả về đúng định dạng để không bị lỗi .cpu()
    return Y if isinstance(Y, np.ndarray) else Y.cpu().numpy()
